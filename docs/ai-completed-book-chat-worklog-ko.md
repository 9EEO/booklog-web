...
사용자는 추천 질문을 누르거나 직접 질문을 입력할 수 있고, AI는 답변과 함께 근거 데이터, 후속 질문을 반환한다.

현재 UI에서는 답변 본문과 후속 질문을 먼저 노출한다. `evidence`는 서버 응답과 메시지 상태에 포함되지만, 근거 카드 UI는 화면 밀도와 모바일 가독성을 우선해 보류했다.
...
다만 이 구조를 모든 질문에 강제하지는 않는다. 저자/작가 배경처럼 외부 정보 자체를 묻는 질문에는 `내 기록에서 본 점`을 억지로 만들지 않고, 사용자가 내 기록과 연결해 달라고 요청했을 때만 개인 기록 연결 섹션을 만든다. 한 문장 리뷰나 짧은 요약처럼 결과 형식이 중요한 질문은 섹션 제목 없이 바로 답하도록 프롬프트를 조정했다.
...
6. 답변, 근거 데이터, 후속 질문을 JSON으로 반환한다.
...
AI 회고 카드, 채팅 바텀시트, 에러 메시지 스타일을 추가했다.

최근 UI 검토 후 다음을 조정했다.

- 챗봇 바텀시트는 좌우/하단 외곽 여백 없이 표시한다.
- 바텀시트 높이는 `80svh`로 올렸다.
- 바텀시트가 열렸을 때 배경 영역 스크롤을 잠근다.
- 챗봇 본문, 입력창, 추천 질문, 에러 문구는 12px로 맞췄다.
- 추천 질문은 메시지 스크롤 영역 안에 둔다.
- 후속 질문은 가장 최근 AI 답변에만 표시한다.
- 입력창 하단에는 safe-area padding을 반영한다.
- 첫 화면 문구는 "무엇이든 물어보기"보다 "내 독서 기록 기반 회고 정리"로 보이도록 조정했다.
- 근거 카드는 펼쳐 보여주지 않고, `내 기록 n개 활용 · 저장 문장 n개 분석` 같은 한 줄 요약만 노출한다.
...
OPENAI_BOOK_CHAT_LIGHT_MODEL=gpt-5-mini
DATA4LIBRARY_AUTH_KEY=발급받은_도서관_정보나루_인증키
...
- `DATA4LIBRARY_AUTH_KEY`도 서버 route에서만 사용하고 `VITE_` 접두어를 붙이지 않는다.
- `VITE_` 접두어가 붙은 환경변수는 브라우저 번들에 노출될 수 있다.
- 현재 외부 API 키는 서버 route에서만 사용한다.
...
명확히 짧은 생성 작업인 질문은 `OPENAI_BOOK_CHAT_LIGHT_MODEL`을 사용한다.

예:

- 이 책을 한 문장 리뷰로 정리해줘
- SNS에 올릴 짧은 문장으로 줄여줘
- 내 저장 문장 기준 키워드 3개를 뽑아줘

다만 `짧게` 같은 표현은 출력 형식일 뿐 작업 난이도를 보장하지 않는다. 외부 검색이나 비교, 저자/작품 배경, 추론이 필요한 질문은 light 키워드가 있어도 main 모델을 사용한다.

mini classifier는 아직 추가하지 않는다. 호출이 하나 늘어 응답 시간이 늘 수 있고, 현재 단계에서는 명확한 light 질문만 light 모델로 보내고 애매하면 main 모델을 쓰는 보수적 라우팅이 더 단순하고 안전하다.

### 답변 구조 선택

답변 구조는 별도 classifier나 하드코딩 분류를 추가하지 않고, main 프롬프트에서 질문 의도에 따라 선택하도록 한다.

- 회고/감상/저장 문장/독서 패턴 질문: 내 기록 중심으로 답한다.
- 작가/저자/작품 배경 질문: 외부 정보 중심으로 답하고 `내 기록에서 본 점`을 만들지 않는다.
- 외부 정보와 내 기록을 연결해 달라는 질문: 외부 정보와 개인 기록 연결을 함께 답한다.
- 한 문장 리뷰/짧은 요약/키워드 추출: 섹션 제목 없이 바로 결과를 답한다.

이 방식은 하드코딩 라우터보다 유연하고, mini classifier처럼 호출을 추가하지 않아 지연을 늘리지 않는다. 다만 반복적인 오분류가 확인되면 이후 mini classifier를 다시 검토한다.
...
ISBN이 있고 `DATA4LIBRARY_AUTH_KEY`가 설정되어 있으면, 도서관 정보나루 API를 참고 데이터로 사용한다.
...
- 모델 라우팅도 키워드 기반이며, 명확한 light 질문만 `gpt-5-mini` 계열로 보낸다.
- mini classifier는 보류했다.
- 대화 기록은 아직 DB에 저장하지 않는다.
- 저장 문장이 매우 많아질 경우 컨텍스트가 커질 수 있다.
- 외부 검색 결과의 출처 표시 UI는 아직 별도로 노출하지 않는다.
...
따라서 외부 검색은 모든 질문에 켜지 않고, 필요한 질문에서만 조건부로 켜는 구조로 구현했다. 독서모임 의견, 리뷰, 감상, 저장 문장 분석처럼 내부 기록만으로 충분한 질문은 빠른 1회 호출을 우선한다.

`리뷰` 키워드는 외부 검색 트리거에서 제외했다. `이 책을 한 문장 리뷰로 정리해줘` 같은 질문은 내 기록 기반 짧은 생성으로 처리하는 편이 빠르고 신뢰도에도 맞다.

이후 외부 검색 트리거에서 `독서모임`, `토론`, 단독 `추천`, 단독 `작품`처럼 내부 기록 답변으로 충분할 수 있는 넓은 키워드는 제외했다. 외부 검색은 저자/작가, 작품 배경, 시대적 배경, 수상, 출판처럼 외부 정보 의도가 비교적 분명할 때만 사용한다. 비슷한 책이나 다음 책 추천은 ISBN이 있으면 도서관 정보나루 참고 데이터를 우선 사용한다.
