...
const clampNumber = (value: number, min: number, max: number) =>
  Math.min(Math.max(Math.floor(value) || min, min), max);
...
const findHighlightForRecord = (book: Book, record: ReadingRecord) => {
  ...
  return book.sentences.find(
    (sentence) =>
      ...
      sentence.page === sentencePage &&
      sentence.recordedAt === record.date,
  );
};
...
const getActiveRound = (book: Book) =>
  ...
  [...getBookRounds(book)].sort(
    (left, right) => right.roundNumber - left.roundNumber,
  )[0];

const getNextRoundNumber = (book: Book) =>
  Math.max(0, ...getBookRounds(book).map((round) => round.roundNumber)) + 1;

const applyActiveRoundToBook = (book: Book, round: ReadingRound): Book => {
  const rounds = getBookRounds(book).map((item) =>
    item.id === round.id ? round : item,
  );
  const nextRounds = rounds.some((item) => item.id === round.id)
    ? rounds
    : [...rounds, round];
  const latestCompletedAt =
    [...nextRounds].reverse().find((item) => item.completedAt)?.completedAt ??
    book.completedAt;
...
const removeBookRound = (
  book: Book,
  roundId: string,
  nextActiveRound: ReadingRound,
): Book => {
  const nextRounds = getBookRounds(book).filter(
    (round) => round.id !== roundId,
  );
  const latestCompletedAt =
    [...nextRounds].reverse().find((round) => round.completedAt)?.completedAt ??
    book.completedAt;
...
function AuthenticatedApp({
  ...
  const [currentBookId, setCurrentBookId] = useState(() =>
    getInitialCurrentBookId(getInitialBooks()),
  );
    ...
    const loadSnapshot = async () => {
      ...
      try {
        ...
        const hasRemoteData =
          ...
          Object.values(remoteSnapshot.tierBoard).some(
        ...
        const hasLocalData =
          ...
          Object.values(localSnapshot.tierBoard).some(
            (bookIds) => bookIds.length > 0,
          );

        const nextSnapshot =
          !hasRemoteData && hasLocalData && localDataOwnerUserId === user.id
            ? await migrateLocalSnapshotToSupabase(
                user.id,
                localSnapshot,
                bookPalettes,
              )
        ...
        setCurrentBookId(
          nextSnapshot.currentBookId ||
            getInitialCurrentBookId(nextSnapshot.books),
        );
  ...
  const handleChangeTierBoard = (nextTierBoard: SetStateAction<TierBoard>) => {
    setTierBoard((current) => {
      const next =
        typeof nextTierBoard === "function"
          ? nextTierBoard(current)
          : nextTierBoard;
  ...
  const handleUpdateRecord = async (
    ...
    const targetRound =
      targetBook && targetRecord.roundId
        ? getBookRounds(targetBook).find(
            (round) => round.id === targetRecord.roundId,
          )
        : targetBook
          ? (getBookRounds(targetBook).find(
              (round) => round.roundNumber === (targetRecord.roundNumber ?? 1),
            ) ?? getActiveRound(targetBook))
          : undefined;
    const totalPages =
      targetBook?.totalPages ??
      Math.max(targetRecord.endPage, input.endPage, 1);
    ...
    const durationSeconds = Math.max(
      Math.floor(input.durationSeconds) || 60,
      60,
    );
    const sentence = input.sentence?.trim();
    const sentencePage = sentence
      ? clampNumber(input.sentencePage ?? endPage, 1, totalPages)
      : undefined;
    const recalculatedEndedAt = addSecondsToIsoDate(
      targetRecord.startedAt,
      durationSeconds,
    );
    ...
    try {
      ...
      const targetHighlight = targetBook
        ? findHighlightForRecord(targetBook, targetRecord)
        : undefined;
      const updatedHighlight =
        targetBook && sentence
          ...
          : null;
      const deletedHighlightId =
        !sentence && targetHighlight ? targetHighlight.id : null;
      ...
      const nextRecords = sortRecordsByRecent(
        records.map((record) =>
      ...
      if (targetBook) {
        const durationDelta =
          updatedRecord.durationSeconds - targetRecord.durationSeconds;
        const roundRecordFilter = (record: ReadingRecord) =>
          record.bookId === targetBook.id &&
          (targetRound?.id
            ? record.roundId === targetRound.id
            : (record.roundNumber ?? 1) ===
              (targetRound?.roundNumber ?? updatedRecord.roundNumber ?? 1));
        const roundRecords = nextRecords.filter(roundRecordFilter);
        const maxRecordedPage = Math.max(
          updatedRecord.endPage,
          ...roundRecords.map((record) => record.endPage),
        );
        const isActiveRound = Boolean(
          targetRound && targetRound.id === targetBook.activeRoundId,
        );
        const nextRoundCurrentPage =
          targetRound && targetRound.currentPage === targetRecord.endPage
            ...
            : clampBookPage(
                Math.max(
                  targetRound?.currentPage ?? targetBook.currentPage,
                  updatedRecord.endPage,
                ),
                targetBook.totalPages,
              );
        const nextCurrentPage = isActiveRound
          ? nextRoundCurrentPage
          : targetBook.currentPage;
        const nextAccumulatedSeconds = Math.max(
          (targetRound?.accumulatedSeconds ?? targetBook.accumulatedSeconds) +
            durationDelta,
          0,
        );
        const isCompleted = isBookCompletedByPage(
        ...
        const nextRoundCompleted = isBookCompletedByPage(
          nextRoundCurrentPage,
          targetBook.totalPages,
        );
        const nextRoundStatus: BookStatus = nextRoundCompleted
          ? "completed"
          : "reading";
        const nextRound = targetRound
          ? {
              ...
              status: nextRoundStatus,
              completedAt: nextRoundCompleted
                ? (targetRound.completedAt ?? updatedRecord.date)
                : undefined,
            }
          : null;
        ...
        if (nextRound && !nextRound.id.includes("-round-")) {
          await updateRemoteReadingRound(nextRound.id, {
            ...
            completedAt:
              nextRound.status === "completed"
                ? (nextRound.completedAt ?? updatedRecord.date)
                : null,
          });
        }
        ...
        if (isActiveRound) {
          await updateRemoteBook(targetBook.id, {
            ...
            completedAt: isCompleted
              ? (targetBook.completedAt ?? updatedRecord.date)
            ...
            book.id === targetBook.id
              ? (() => {
                  const nextBook = nextRound
                    ? isActiveRound
                      ? applyActiveRoundToBook(book, nextRound)
                      : replaceBookRound(book, nextRound)
                    : book;
                  ...
                  return {
                    ...
                    sentences: updatedHighlight
                      ? book.sentences.some(
                          (sentenceItem) =>
                            sentenceItem.id === updatedHighlight.id,
                        )
                        ? book.sentences.map((sentenceItem) =>
                            sentenceItem.id === updatedHighlight.id
                              ? updatedHighlight
                              : sentenceItem,
                          )
                      ...
                      : deletedHighlightId
                        ? book.sentences.filter(
                            (sentenceItem) =>
                              sentenceItem.id !== deletedHighlightId,
                          )
                        : book.sentences,
                  };
                })()
      ...
      if (targetBook) {
        const remainingBookRecords = nextRecords.filter(
          (record) =>
            ...
            (targetRound?.id
              ? record.roundId === targetRound.id
              : (record.roundNumber ?? 1) === (targetRound?.roundNumber ?? 1)),
        );
        const fallbackPage = Math.max(
          targetRecord.startPage,
          ...remainingBookRecords.map((record) => record.endPage),
          1,
        );
        const isActiveRound = Boolean(
          targetRound && targetRound.id === targetBook.activeRoundId,
        ...
        const nextRoundCurrentPage =
          targetRound?.currentPage === targetRecord.endPage
            ? clampBookPage(fallbackPage, targetBook.totalPages)
            : (targetRound?.currentPage ?? targetBook.currentPage);
        const nextCurrentPage = isActiveRound
          ? nextRoundCurrentPage
          : targetBook.currentPage;
        ...
        const nextRound = targetRound
          ? {
              ...
              status: nextRoundStatus,
              completedAt: nextRoundCompleted
                ? (targetRound.completedAt ?? targetRecord.date)
                : undefined,
            }
          : null;
        ...
        if (nextRound && !nextRound.id.includes("-round-")) {
          await updateRemoteReadingRound(nextRound.id, {
            ...
            completedAt:
              nextRound.status === "completed"
                ? (nextRound.completedAt ?? targetRecord.date)
                : null,
          });
        }
        ...
        if (isActiveRound) {
          await updateRemoteBook(targetBook.id, {
            ...
            status: isCompleted ? "completed" : "reading",
            completedAt: isCompleted
              ? (targetBook.completedAt ?? targetRecord.date)
              : (targetBook.completedAt ?? null),
          });
        }
              ...
              ? {
                  ...(nextRound
                    ? isActiveRound
                      ...
                      : replaceBookRound(book, nextRound)
                    : book),
                  sentences: targetHighlight
                    ? book.sentences.filter(
                        (sentence) => sentence.id !== targetHighlight.id,
                      )
                    : book.sentences,
                }
              : book,
      ...
      setBooks((current) =>
        current.map((book) =>
          book.id === bookId
            ? {
                ...book,
                sentences: book.sentences.filter(
  ...
  const handleStartReread = async (bookId: string) => {
    ...
    try {
      ...
      setBooks((current) =>
        current.map((book) =>
          book.id === bookId ? applyActiveRoundToBook(book, nextRound) : book,
        ),
      );
  ...
  const handleDeleteRound = async (bookId: string, roundId: string) => {
    const targetBook = books.find((book) => book.id === bookId);
    const targetRound = targetBook
      ? getBookRounds(targetBook).find((round) => round.id === roundId)
      : undefined;
    if (!targetBook || !targetRound || targetRound.roundNumber <= 1) return;

    const remainingRounds = getBookRounds(targetBook).filter(
      (round) => round.id !== roundId,
    );
    const nextActiveRound =
      remainingRounds.find((round) => round.status === "reading") ??
      [...remainingRounds].sort(
        (left, right) => right.roundNumber - left.roundNumber,
      )[0];
    ...
    try {
      ...
      const roundRecords = records.filter(
        (record) => record.bookId === bookId && record.roundId === roundId,
      );
      ...
      await Promise.all(
        [...highlightIdsToDelete].map((highlightId) =>
          deleteRemoteHighlight(highlightId),
        ),
      );
      ...
      await updateRemoteBook(bookId, {
        ...
        status: nextActiveRound.status,
        completedAt:
          nextActiveRound.completedAt ?? targetBook.completedAt ?? null,
      });
        ...
        current.map((book) =>
          book.id === bookId
            ? {
                ...
                sentences: book.sentences.filter(
                  (sentence) => !highlightIdsToDelete.has(sentence.id),
                ),
              }
            : book,
        ),
      ...
      if (currentBookId === bookId) {
        const fallbackReadingBook = books.find(
          (book) => book.id !== bookId && book.status === "reading",
        );
        setCurrentBookId(
          nextActiveRound.status === "reading"
            ? bookId
            : (fallbackReadingBook?.id ?? bookId),
        );
        readingTimer.reset();
      }
    } catch (error) {
  ...
  const handleDeleteBook = async (bookId: string) => {
    try {
      ...
      if (currentBookId === bookId) {
        const nextBook =
          remainingBooks.find((book) => book.status === "reading") ??
          remainingBooks[0];
